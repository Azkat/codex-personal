...
- Obsidianの今日のデイリーメモ
- YouTubeのお気に入り動画（YouTube API上は高評価プレイリスト）
- InoreaderからエクスポートしたOPML/RSS
- Raindropの直近24時間ブックマーク
...
このプロジェクトのコード、README、requirements、launchd、セットアップスクリプトは
`~/.codex/skills` のGitリポジトリで管理します。OAuthトークン、APIキー、OPML、
取得結果キャッシュ、ログ、仮想環境はローカル専用で、GitHubには載せません。
...
- Obsidianメモ要約
- RSS選抜10件の表
- YouTubeお気に入り動画の表（取得元はYouTubeの高評価プレイリスト）
- Raindrop直近24時間ブックマーク全件の表
- Slackへの短縮版通知
